...
for i = 1:2:numel(v)
    ...
    switch lower(v{i})
        ...
        case 'save_option'
            options.save_option = getParam(v,i);
        case 'folder_destination'
            options.folder_destination = getParam(v,i);
        otherwise
            param = getParam(v,i);
...
if isempty(options.folder_destination)
    warning('CIDRE:parseInput', 'No destination folder was specified. CIDRE will load corrected images into workspace.\n');
end
...
if i+1 <= numel(v)
    if isnumeric(v{i+1}) || ischar(v{i+1})
        param = v{i+1};
    else 
        warning('CIDRE:parseInput', 'Expected numeric or char value\n');
    end
end
