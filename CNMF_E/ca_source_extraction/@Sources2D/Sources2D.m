classdef Sources2D < handle
    ...
    properties
        ...
        C_df;       % inferred temporal components of neurons and background normalized by Df
        C_raw_df;   % raw temporal components of neurons and background normalized by Df
    ...
    methods
        ...
        function nam = select_data(obj, nam)
            %% select file
            if ~exist('nam', 'var') || isempty(nam) || ~isfile(nam)     % ----- modified by SR (2020-11-22)-----
        ...
        function sn = estimate_noise(obj, frame_range, method)
            ...
            fprintf('estimating the noise level for every pixel.......\n');
            tic;
            ...
            fprintf('Time cost for estimating the noise levels:  %.3f \n\n', toc);
            sn = cell2mat(sn);
        end
        ...
        %------------------------------------------------------------------EXPORT---%
        function save_neurons_cnmfe(obj, folder_nm, with_craw)
        ...
        %% save 'neuron' object as a structure and all base workspace variables for quick loading
        function file_path = saveall_to_struct(obj)
            ...
            save(file_path, varname); 
            fprintf(sprintf('neuron structure is saved in: \n%s\n',file_path));
        end
