function figh = fig_pintomonitor(figh, varargin)
    ...
    %       figh                : handle to fig - can be empty ('').  (default: gcf)
    %       which_monitor       : (default: 'current' )
    %       units               : (default: 'normalized')
    %       aspect_ratio_x_y    : 4/3 (default)
    %       fracx               : fraction of monitor width to scale figure. (default: 0.4)
    %       
    
    % Author: Suva Roy (June, 2020)
    ...
    set(figh,'outerposition',figouterpos,'color','w');
    figh;
end
