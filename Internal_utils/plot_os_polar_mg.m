function plot_os_polar_mg(stim, StimComb, resp, savepath,  print_out)
...
name{1} = strcat(savepath,datname,'MG_OSpolar');
name{2} = strcat(savepath,datname,'MG_OSpolar_shaded');
        ...
        for cc=1:length(cell_ids)
            ...
            for dr=1:num_dirs
                ...
                for ntr=1:stim.repetitions_corrected
                    localtrace = resp.unique_tp(1).unique_sp(1).unique_contrast(1).unique_dirs(dr).cell(cell_ids(cc)).raster_unfilt_traces(ntr,:);
            ...
            tt_rho = resp.os_estimates.rho(cell_ids(cc),:);
            tt_rho = [tt_rho tt_rho(1)];
            arrow_ang = resp.os_estimates.angle(cell_ids(cc));
            ...
            osi = resp.os_estimates.os_index(cell_ids(cc));
            sgtitle(sprintf('%s stim. OSI = %1.2f. Cell id %d',stim.type, osi, cell_ids(cc)));
            ...
            for dr=1:num_dirs
                ...
                for ntr=1:stim.repetitions_corrected
                    localtrace = [localtrace; ...
                        resp.unique_tp(1).unique_sp(1).unique_contrast(1).unique_dirs(dr).cell(cell_ids(cc)).raster_unfilt_traces(ntr,:)];
                end
