function stim = get_stimprops(stim, dataname, initpath)
    ...
    if ~strcmpi(stim.type,'none')
        ...
        lookfor_fulln1 = strcat('s0',lookfor_n);
        lookfor_fulln2 = strcat('s',lookfor_n);
        cnt = 1; 
        for ii=1:length(lcc)
            if ~isempty(regexpi(fnames(lcc(ii)).name,'.m')) && ~isempty(regexpi(fnames(lcc(ii)).name,'.mat')) && ...
                    (~isempty(regexpi(fnames(lcc(ii)).name,lookfor_fulln1)) || ~isempty(regexpi(fnames(lcc(ii)).name,lookfor_fulln2)))
